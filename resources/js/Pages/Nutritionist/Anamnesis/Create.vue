...
import { ArrowLeft, Plus, Trash2, GripVertical, ChevronUp, ChevronDown } from 'lucide-vue-next';
import { ref } from 'vue';
...
const questionTypes: { value: QuestionType; label: string; desc: string }[] = [
    { value: 'text',     label: 'Testo breve',    desc: 'Risposta in una riga' },
    { value: 'textarea', label: 'Testo lungo',     desc: 'Risposta su più righe' },
    { value: 'number',   label: 'Numero',          desc: 'Valore numerico (peso, età...)' },
    { value: 'scale',    label: 'Scala 1-10',      desc: 'Valutazione su scala numerica' },
    { value: 'radio',    label: 'Scelta singola',  desc: 'Una sola risposta tra le opzioni' },
    { value: 'checkbox', label: 'Scelta multipla', desc: 'Più risposte possibili' },
];
...
function submit() {
    form.post(route('nutritionist.anamnesis.store'));
}

const typeLabel = (t: QuestionType) => questionTypes.find(q => q.value === t)?.label ?? t;
</script>

<template>
    <Head title="Nuovo Template Anamnesi" />
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center gap-3">
                ...
                <h1 class="text-xl font-semibold text-gray-900">Nuovo Template Anamnesi</h1>
            </div>
        </template>
        ...
        <form @submit.prevent="submit" class="max-w-3xl space-y-6">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input
                            ...
                            type="text"
                            required
                            placeholder="es. Anamnesi prima visita"
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            rows="2"
                            placeholder="Breve descrizione di quando usare questo template..."
                            class="w-full rounded-lg border-gray-300 text-sm focus:border-primary-500 focus:ring-primary-500"
                        ></textarea>
                    </div>
            ...
            <div class="rounded-2xl bg-white border border-gray-100 shadow-sm p-6">
                ...
                <div v-if="form.questions.length === 0" class="text-center py-8 text-gray-400 border-2 border-dashed border-gray-200 rounded-xl mb-4">
                    <p class="text-sm">Nessuna domanda ancora. Aggiungine una!</p>
                </div>
                            ...
                            <div class="flex-1 space-y-3">
                                ...
                                <div class="flex flex-wrap items-center gap-3">
                                    <select
                                        ...
                                        class="rounded-lg border-gray-200 text-xs py-1.5 focus:border-primary-500 focus:ring-primary-500"
                                    >
                                ...
                                <div v-if="q.type === 'radio' || q.type === 'checkbox'" class="space-y-2">
                                    <p class="text-xs text-gray-500 font-medium">Opzioni di risposta:</p>
                                    <div v-for="(opt, oi) in q.options" :key="oi" class="flex items-center gap-2">
                                        ...
                                        <input
                                            ...
                                            class="flex-1 rounded-lg border-gray-200 text-xs py-1.5 focus:border-primary-500 focus:ring-primary-500"
                                        />
                                        <button type="button" @click="removeOption(q, oi)" class="text-gray-300 hover:text-red-400">
                                ...
                                <div v-if="q.type === 'scale'" class="flex gap-1">
                                    <span
                                        v-for="n in 10"
                                        :key="n"
                                        class="flex h-7 w-7 items-center justify-center rounded-lg border border-gray-200 text-xs text-gray-400"
                                    >{{ n }}</span>
                                </div>
                            </div>
                            <button
