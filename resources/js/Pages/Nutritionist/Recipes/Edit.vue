...
import { ArrowLeft, Trash2, Search, Flame, Beef, Wheat, Droplets } from 'lucide-vue-next';
...
function addIngredient(food: typeof props.foods[0]) {
    if (form.ingredients.some((i: any) => i.food_id === food.id)) return;
...
function submit() {
    form.put(route('nutritionist.recipes.update', props.recipe.id));
}
</script>

<template>
    <Head :title="`Modifica ${recipe.name}`" />
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center gap-3">
                <Link :href="route('nutritionist.recipes.show', recipe.id)" class="rounded-lg p-1.5 text-gray-400 hover:bg-gray-100 hover:text-gray-600 transition">
                    <ArrowLeft class="h-5 w-5" />
                </Link>
                <h1 class="text-xl font-semibold text-gray-900">Modifica {{ recipe.name }}</h1>
            </div>
        </template>
        ...
        <form @submit.prevent="submit" class="max-w-3xl">
                ...
                <div class="relative mb-4">
                    ...
                    <div v-if="showFoodDropdown && filteredFoods.length > 0" class="absolute z-10 mt-1 w-full rounded-lg bg-white border border-gray-200 shadow-lg max-h-60 overflow-y-auto">
                        <button
                            ...
                            :class="{ 'opacity-50 cursor-not-allowed': form.ingredients.some((i: any) => i.food_id === food.id) }"
                        >
            ...
            <div class="flex items-center gap-3">
                ...
                <Link :href="route('nutritionist.recipes.show', recipe.id)" class="text-sm text-gray-600 hover:text-gray-900">Annulla</Link>
            </div>
        </form>
    </AuthenticatedLayout>
</template>
