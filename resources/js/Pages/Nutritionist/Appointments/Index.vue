...
import {
    ...
    UserPlus, RefreshCw, Video, MoreHorizontal, Check, ChevronRight as Next, Ban,
...
// ─── View mode ───────────────────────────────────────────────────────────────
const viewMode = ref<'month' | 'week' | 'day' | 'list'>(
    (localStorage.getItem('calendar-default-view') as 'month' | 'week' | 'day' | 'list' | null) || 'month'
)

const pendingDefaultView = ref<'month' | 'week' | 'day' | 'list' | null>(null)
const viewLabels: Record<string, string> = { month: 'Mese', week: 'Settimana', day: 'Giorno', list: 'Lista' }

function changeView(v: 'month' | 'week' | 'day' | 'list') {
    if (v === viewMode.value) return
    // Switching from month/list to week/day: anchor to today if we're in the current month
    if ((v === 'week' || v === 'day') && (viewMode.value === 'month' || viewMode.value === 'list')) {
    ...
    viewMode.value = v
    if (v === 'week' || v === 'day') nextTick(scrollToCurrentTime)
    const saved = localStorage.getItem('calendar-default-view') || 'month'
    if (v !== saved) pendingDefaultView.value = v
}

function confirmDefaultView() {
    if (!pendingDefaultView.value) return
    localStorage.setItem('calendar-default-view', pendingDefaultView.value)
    pendingDefaultView.value = null
}

function dismissDefaultView() {
    pendingDefaultView.value = null
}

const showWeekend = ref(localStorage.getItem('calendar-show-weekend') !== 'false')
const pendingWeekendPref = ref(false)

function toggleWeekend() {
    showWeekend.value = !showWeekend.value
    const savedVal = localStorage.getItem('calendar-show-weekend') !== 'false'
    if (showWeekend.value !== savedVal) pendingWeekendPref.value = true
}

function confirmWeekendPref() {
    localStorage.setItem('calendar-show-weekend', String(showWeekend.value))
    pendingWeekendPref.value = false
}

function dismissWeekendPref() {
    pendingWeekendPref.value = false
}
...
// ─── Week view: compact layout with off-hours toggle ─────────────────────────
const WEEK_HOUR_HEIGHT = 48;
const WORK_START = 8;
const WORK_END   = 20;
const showOffHours = ref(false);

const weekAptBounds = computed(() => {
    const all = weekDaysWithDates.value.flatMap(d => appointmentsForDay(d));
    if (!all.length) return { min: WORK_START, max: WORK_END };
    const starts = all.map(a => new Date(a.starts_at).getHours());
    const ends   = all.map(a => {
        const d = new Date(a.ends_at);
        return d.getHours() + (d.getMinutes() > 0 ? 1 : 0);
    });
    return { min: Math.min(WORK_START, ...starts), max: Math.max(WORK_END, ...ends) };
});

const weekViewStart   = computed(() => showOffHours.value ? 0 : weekAptBounds.value.min);
const weekViewEnd     = computed(() => showOffHours.value ? 24 : weekAptBounds.value.max);
const weekViewTotalPx = computed(() => (weekViewEnd.value - weekViewStart.value) * WEEK_HOUR_HEIGHT);

const weekViewHours = computed(() => {
    const h: number[] = [];
    for (let i = weekViewStart.value; i <= weekViewEnd.value; i++) h.push(i);
    return h;
});

const weekViewSlots = computed(() => {
    const slots: { key: string; hour: number; minute: number; timeStr: string }[] = [];
    for (let h = weekViewStart.value; h < weekViewEnd.value; h++) {
        for (let m = 0; m < 60; m += 15) {
            slots.push({ key: `${h}-${m}`, hour: h, minute: m, timeStr: `${String(h).padStart(2,'0')}:${String(m).padStart(2,'0')}` });
        }
    }
    return slots;
});

const weekCurrentTimePx = computed(() => {
    const now = new Date();
    return (now.getHours() + now.getMinutes() / 60 - weekViewStart.value) * WEEK_HOUR_HEIGHT;
});

function weekAptTopPx(apt: any): number {
    const d = new Date(apt.starts_at);
    return Math.max(0, (d.getHours() + d.getMinutes() / 60 - weekViewStart.value) * WEEK_HOUR_HEIGHT);
}

function weekAptHeightPx(apt: any): number {
    const start = new Date(apt.starts_at);
    const end   = new Date(apt.ends_at);
    const mins  = Math.max((end.getTime() - start.getTime()) / 60000, 20);
    return (mins / 60) * WEEK_HOUR_HEIGHT;
}
...
function aptBgColor(status: string, type?: string): string {
    if (type === 'blocked') return 'apt-blocked border-l-[3px] border-slate-400 text-slate-600';
...
function scrollToCurrentTime() {
    ...
    if (viewMode.value === 'week') {
        const targetH = Math.max(weekViewStart.value, now.getHours() - 1);
        timeGridRef.value.scrollTop = (targetH - weekViewStart.value) * WEEK_HOUR_HEIGHT;
    } else {
        timeGridRef.value.scrollTop = Math.max(7, now.getHours() - 1) * HOUR_HEIGHT;
    }
}
...
function onDragStart(e: DragEvent, apt: any) {
    ...
    const hh = viewMode.value === 'week' ? WEEK_HOUR_HEIGHT : HOUR_HEIGHT;
    dragOffsetMinutes.value = Math.round((e.offsetY / hh) * 60 / 15) * 15;
    if (e.dataTransfer) {
...
function slotId(day: Date, slot: { hour: number; minute: number }): string {
    return `${localDateStr(day)}-${slot.hour}-${slot.minute}`;
}
...
function onSlotDrop(e: DragEvent, day: Date, slot: { hour: number; minute: number; timeStr: string }) {
    ...
    const newStartDate = new Date(day.getFullYear(), day.getMonth(), day.getDate(), startHour, startMin, 0, 0);
    const newStartsAt  = newStartDate.toISOString();
    const origDuration = new Date(apt.ends_at).getTime() - new Date(apt.starts_at).getTime();
    const newEndsAt    = new Date(newStartDate.getTime() + origDuration).toISOString();
...
function appointmentsForDay(date: Date | null): any[] {
    ...
    return props.appointments.data.filter((a: any) => localDateStr(new Date(a.starts_at)) === ds);
}
...
function confirmTypeAndAdvance() {
    ...
    // Blocked slots have no client — skip step 2
    wizardStep.value = wizard.type === 'blocked' ? 3 : 2;
}
...
function openWizard(prefillDate?: Date, prefillTime?: string) {
    ...
    wizard.notes            = '';
    wizard.custom_title     = '';
    wizard.date             = localDateStr(prefillDate ?? new Date());
...
function submitWizard() {
    ...
    const [sy, sm, sd] = wizard.date.split('-').map(Number);
    const [sh, smin]   = wizard.start_time.split(':').map(Number);
    const [eh, emin]   = wizard.end_time.split(':').map(Number);
    ...
    const data: Record<string, any> = {
        ...
        notes:        wizard.notes,
        custom_title: wizard.custom_title || undefined,
    };
...
const typeColorMap: Record<string, { bg: string; border: string; icon: string; selBg: string; selBorder: string }> = {
    ...
    blocked:     { bg: 'bg-slate-50',   border: 'border-slate-300',   icon: 'text-slate-500',   selBg: 'bg-slate-100',  selBorder: 'border-slate-500'   },
};
    ...
    <AuthenticatedLayout>
            ...
            <div class="flex items-center justify-between flex-wrap gap-2">
                ...
                <div class="flex items-center gap-2 flex-wrap">
                    ...
                    <div class="flex rounded-lg border border-gray-200 overflow-hidden">
                        <button @click="changeView('month')" :class="['flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium transition', viewMode === 'month' ? 'bg-primary-500 text-white' : 'text-gray-600 hover:bg-gray-50']">
                            <Calendar class="h-4 w-4" /> Mese
                        </button>
                        <button @click="changeView('week')" :class="['flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium border-l border-gray-200 transition', viewMode === 'week' ? 'bg-primary-500 text-white' : 'text-gray-600 hover:bg-gray-50']">
                            <CalendarDays class="h-4 w-4" /> Settimana
                        </button>
                        <button @click="changeView('day')" :class="['flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium border-l border-gray-200 transition', viewMode === 'day' ? 'bg-primary-500 text-white' : 'text-gray-600 hover:bg-gray-50']">
                            <Clock class="h-4 w-4" /> Giorno
                        </button>
                        <button @click="changeView('list')" :class="['flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium border-l border-gray-200 transition', viewMode === 'list' ? 'bg-primary-500 text-white' : 'text-gray-600 hover:bg-gray-50']">
        ...
        <template v-else-if="viewMode === 'week'">
            <div class="flex items-center justify-between mb-4">
                ...
                <div class="flex items-center gap-3">
                    <button
                        @click="showOffHours = !showOffHours"
                        :class="['rounded-lg px-2.5 py-1.5 text-xs font-medium border transition flex items-center gap-1.5',
                            showOffHours ? 'border-primary-300 bg-primary-50 text-primary-700' : 'border-gray-200 text-gray-400 hover:bg-gray-50 hover:text-gray-600']"
                    >
                        <Clock class="h-3.5 w-3.5" />
                        {{ showOffHours ? 'Orario lavorativo' : 'Tutte le ore' }}
                    </button>
                    <button @click="goToToday" class="rounded-lg px-3 py-1.5 text-sm font-medium border border-gray-200 text-gray-600 hover:bg-gray-50 transition">Oggi</button>
                        ...
                        <div class="flex-1" :style="gridStyle(weekDaysWithDates.length)">
                            ...
                            >
                                ...
                                <div
                                    ...
                                    :style="`top: ${(slot.hour - weekViewStart + slot.minute / 60) * WEEK_HOUR_HEIGHT}px; height: ${WEEK_HOUR_HEIGHT / 4}px`"
                                ...
                                <div
                                    ...
                                    :style="`top: ${weekCurrentTimePx}px`"
                                >
                                ...
                                <div
                                    ...
                                    :class="[aptBgColor(slot.apt.status, slot.apt.type), draggingApt?.id === slot.apt.id ? 'opacity-40' : '']"
                                    :style="`top: ${weekAptTopPx(slot.apt)}px; height: ${weekAptHeightPx(slot.apt)}px; min-height: 22px; left: calc(${(slot.col / slot.totalCols) * 100}% + 2px); width: calc(${100 / slot.totalCols}% - 4px)`"
                                    draggable="true"
        ...
        <template v-else>
            <div class="mb-6 flex flex-col sm:flex-row gap-3">
                <input
                    type="month"
                    :value="props.filters.month || `${new Date().getFullYear()}-${String(new Date().getMonth()+1).padStart(2,'0')}`"
        ...
        <div v-if="showWizard" class="fixed inset-0 z-50 flex items-center justify-center bg-black/30" @click.self="showWizard = false">
            <div class="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4">
                ...
                <div class="flex items-center justify-between px-6 pt-5 pb-4 border-b border-gray-100">
                    <div class="flex items-center gap-3">
                        <button v-if="wizardStep > 1" type="button"
                            @click="wizardStep = (wizardStep === 3 && wizard.type === 'blocked' ? 1 : wizardStep - 1) as 1 | 2 | 3"
                            class="p-1.5 rounded-lg text-gray-400 hover:bg-gray-100 transition">
                            <ChevronLeft class="h-4 w-4" />
                        </button>
                        <div>
                            <p class="text-xs font-medium text-gray-400 uppercase tracking-wide">
                                Passo {{ wizardStep === 3 && wizard.type === 'blocked' ? 2 : wizardStep }} di {{ wizard.type === 'blocked' ? 2 : 3 }}
                            </p>
                ...
                <div class="flex gap-1.5 px-6 pt-3 pb-1">
                    <div v-for="s in (wizard.type === 'blocked' ? 2 : 3)" :key="s"
                        :class="['h-1 flex-1 rounded-full transition-colors duration-300',
                            (wizard.type === 'blocked' ? (wizardStep === 3 ? 2 : wizardStep) : wizardStep) >= s ? 'bg-green-500' : 'bg-gray-200']">
                    </div>
                </div>
                ...
                <!-- Step 2: Cliente -->
                <div v-else-if="wizardStep === 2" class="px-6 pb-6 pt-4 space-y-3">
                    <!-- "Nuovo cliente" prominent button — BEFORE the search list -->
                    <button
                        v-if="!wizard.new_client"
                        type="button"
                        @click="startNewClient"
                        class="w-full flex items-center gap-2 px-4 py-2.5 rounded-xl border-2 border-dashed border-green-200 text-sm text-green-600 hover:border-green-400 hover:bg-green-50 transition font-medium"
                    >
                        <Plus class="h-4 w-4 flex-shrink-0" /> Nuovo cliente
                    </button>

                    <!-- Divider -->
                    <div v-if="!wizard.new_client" class="flex items-center gap-2">
                        <div class="flex-1 border-t border-gray-200"></div>
                        <span class="text-xs text-gray-400">oppure scegli esistente</span>
                        <div class="flex-1 border-t border-gray-200"></div>
                    </div>
                ...
                <div v-else-if="wizardStep === 3" class="p-6 space-y-4">
                    <!-- Titolo opzionale (solo per slot occupato) -->
                    <div v-if="wizard.type === 'blocked'">
                        <label class="block text-sm font-medium text-gray-700 mb-1">
                            Titolo <span class="text-gray-400 font-normal">(facoltativo)</span>
                        </label>
                        <input
                            v-model="wizard.custom_title"
                            type="text"
                            placeholder="Es. Riunione, Pausa pranzo…"
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        ...
                        <div class="col-span-1">
                            <label class="block text-sm font-medium text-gray-700 mb-1">Inizio *</label>
                            <select v-model="wizard.start_time" required @change="onWizardStartTimeChange" class="block w-full rounded-lg border-gray-300 text-sm focus:border-primary-500 focus:ring-primary-500">
                                <option v-for="t in timeOptions" :key="t.value" :value="t.value">{{ t.label }}</option>
                            </select>
                        </div>
                        <div class="col-span-1">
                            <label class="block text-sm font-medium text-gray-700 mb-1">Fine *</label>
                            <select v-model="wizard.end_time" required class="block w-full rounded-lg border-gray-300 text-sm focus:border-primary-500 focus:ring-primary-500">
                                <option v-for="t in timeOptions" :key="t.value" :value="t.value">{{ t.label }}</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div v-else>
                        ...
                        <select v-model="wizard.location" class="block w-full rounded-lg border-gray-300 text-sm focus:border-primary-500 focus:ring-primary-500">
        ...
        <!-- Weekend preference toast -->
        <Transition name="slide-up">
            <div
                v-if="pendingWeekendPref && !pendingMove && !pendingDefaultView"
            ...
            >
                <div class="text-sm leading-snug">
                    Weekend <span class="font-semibold">{{ showWeekend ? 'visibili' : 'nascosti' }}</span>
                </div>
                <div class="flex gap-2 flex-shrink-0">
                    <button
                        @click="confirmWeekendPref"
                        class="rounded-lg bg-primary-500 px-4 py-2 text-sm font-medium hover:bg-primary-400 transition"
                    >
                        Salva preferenza
                    </button>
                    <button @click="dismissWeekendPref" class="rounded-lg bg-white/10 px-4 py-2 text-sm hover:bg-white/20 transition">
                        Ignora
                    </button>
                </div>
            </div>
        ...
        <Transition name="slide-up">
            <div
                v-if="pendingDefaultView && !pendingMove"
                class="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-gray-900 text-white rounded-2xl px-5 py-4 shadow-2xl flex items-center gap-4 min-w-max max-w-[calc(100vw-2rem)]"
            >
                <div class="text-sm leading-snug">
                    Vista <span class="font-semibold">{{ viewLabels[pendingDefaultView] }}</span> selezionata
                </div>
                <div class="flex gap-2 flex-shrink-0">
                    <button
                        @click="confirmDefaultView"
                        class="rounded-lg bg-primary-500 px-4 py-2 text-sm font-medium hover:bg-primary-400 transition"
                    >
                        Rendi predefinita
                    </button>
                    <button @click="dismissDefaultView" class="rounded-lg bg-white/10 px-4 py-2 text-sm hover:bg-white/20 transition">
                        Ignora
                    </button>
                </div>
            </div>
