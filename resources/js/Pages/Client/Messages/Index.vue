...
function sendMessage() {
    ...
    form.post(route('client.messages.store'), {
        preserveScroll: true,
        onSuccess: () => {
            form.reset('body');
        ...
        <template #header>
            <div class="flex items-center gap-3">
                ...
                <h1 class="text-lg font-semibold text-gray-900">
                    Messaggi{{ nutritionist ? ` con ${nutritionist.name}` : '' }}
                </h1>
            </div>
        </template>
            ...
            <div ref="chatContainer" class="flex-1 overflow-y-auto px-2 py-4 space-y-1">
                <div v-if="messages.length === 0" class="flex items-center justify-center h-full">
                    <div class="text-center">
                        <MessageSquare class="mx-auto h-10 w-10 text-gray-300 mb-3" />
                        <p class="text-sm text-gray-400">Nessun messaggio. Scrivi al tuo nutrizionista!</p>
                    </div>
                </div>
